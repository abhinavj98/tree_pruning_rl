...
print('Using device:', 'cuda' if args.cuda else 'cpu', ', device number:', args.device_num, ', GPUs in system:',
...
def main():
    ...
    env = ur5GymEnv(renders=args.render, maxSteps=args.mel, actionRepeat=args.repeat, task=args.task,
                    randObjPos=args.randObjPos, simulatedGripper=args.simgrip, learning_param=args.lp)
    ...
    for i_episode in range(1, args.max_episodes + 1):
        ...
        if i_episode % args.save_interval == 0:
            torch.save(ppo.policy.state_dict(),
                       args.save_dir + '/model_epoch_' + str(int(i_episode / args.save_interval)) + '.pth')
